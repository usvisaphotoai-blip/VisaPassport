...
export default function PreviewClientDe({
  ...
  const docCategory = isVisa ? "Visum" : "Passbild";
  const productName = spec?.name || `${docCategory} Foto`;
  ...
  const handleEmailPhoto = async () => {
    const emailTo = window.prompt("Geben Sie Ihre E-Mail-Adresse ein, um das Foto zu erhalten:");
    if (!emailTo) return;
    try {
      ...
      alert(res.ok ? "Foto gesendet! Bitte überprüfen Sie Ihren Posteingang." : "Fehler beim Senden. Bitte versuchen Sie es erneut.");
    } catch { alert("Fehler beim Senden der E-Mail."); }
  };
        ...
        <div className="w-full max-w-6xl">
          <div className="mb-3 text-center lg:text-left">
            <h1 className="text-2xl font-black text-slate-900">
              {de.preview.title} <span className="text-lime-600">{spec?.flag || ""} {spec?.country || ""}</span>
            </h1>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-5">
            ...
            <div className="w-full lg:w-[62%] space-y-4">
              <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-sm">
                ...
                <div className="relative cursor-zoom-in group mx-5 mb-5 rounded-2xl overflow-hidden bg-slate-50 border border-slate-100 flex items-center justify-center" onClick={() => setIsZoomOpen(true)} style={{ minHeight: 240 }}>
                  ...
                  <img src={previewUrl} alt="Vorschau" className="max-h-[420px] w-auto max-w-full object-contain select-none pointer-events-none block" draggable={false} onContextMenu={(e) => e.preventDefault()} />
            ...
            <div className="w-full lg:w-[38%] space-y-4">
                ...
                <div className="p-5 sm:p-6">
                  {!hasPaid ? (
                    <div className="space-y-4">
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">{de.payment.orderSummary} — {de.payment.acceptedFor}</p>
                      {/* Standard plan */}
                      <button onClick={() => setIsExpertPlan(false)} className={cx("w-full text-left rounded-2xl border-2 p-4 transition-all duration-200 relative", !isExpertPlan ? "border-emerald-500 bg-emerald-50/40 shadow-[0_0_0_3px_rgba(16,185,129,0.08)]" : "border-slate-100 bg-white hover:border-slate-200")}>
                        <div className="flex justify-between items-start mb-2">
                          <h4 className="text-sm font-bold text-slate-900 mt-1">{de.payment.standardPack}</h4>
                          <p className="text-xl font-black text-slate-900">{localPrice?.formatted}</p>
                        </div>
                        <ul className="space-y-1.5">
                          {de.payment.standardFeatures.map((f) => (<li key={f} className="flex items-center gap-2 text-[13px] text-slate-600 font-medium"><Icon d={ICONS.check} size={12} className={!isExpertPlan ? "text-emerald-500" : "text-slate-300"} stroke={2.5} />{f}</li>))}
                        </ul>
                      </button>
                      {/* Expert plan */}
                      <button onClick={() => setIsExpertPlan(true)} className={cx("w-full text-left rounded-2xl border-2 p-4 transition-all duration-200 relative", isExpertPlan ? "border-blue-500 bg-blue-50/40 shadow-[0_0_0_3px_rgba(59,91,219,0.08)]" : "border-slate-100 bg-white hover:border-slate-200")}>
                        ...
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            ...
                            <h4 className="text-sm font-bold text-slate-900 mt-1">{de.payment.premiumPack}</h4>
                          </div>
                          <p className="text-xl font-black text-slate-900">{expertPrice?.formatted}</p>
                        </div>
                        <ul className="space-y-1.5">
                          {de.payment.premiumFeatures.map((f) => (<li key={f} className="flex items-center gap-2 text-[14px] text-slate-600 font-medium"><Icon d={ICONS.check} size={12} className={isExpertPlan ? "text-blue-500" : "text-slate-300"} stroke={2.5} />{f}</li>))}
                        </ul>
                        {isExpertPlan && (
                          <div className="mt-3 bg-blue-100/50 rounded-xl px-3 py-2 text-[11px] text-blue-800 font-semibold flex items-center gap-1.5">
                            <Icon d={ICONS.shield} size={12} className="text-blue-600 shrink-0" />{de.payment.moneyBack}
                          </div>
                        )}
                      </button>
                      ...
                      {status !== "authenticated" && (
                        <div>
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">{de.payment.emailForDelivery}</label>
                          <input type="email" value={guestEmail} onChange={(e) => setGuestEmail(e.target.value)} placeholder={de.payment.enterEmail} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all" />
                        </div>
                      )}
                      ...
                      <div className="space-y-2 pt-1">
                        <button onClick={onPaymentClick} disabled={loading}
                          ...
                          {loading ? (<div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />) : (
                            <><Icon d={ICONS.download} size={16} className="shrink-0" />{isExpertPlan ? de.payment.getExpertReview : de.payment.downloadNow} — {isExpertPlan ? expertPrice?.formatted : localPrice?.formatted}</>
                          )}
                        </button>
                        <div className="flex items-center justify-center gap-4 py-2 flex-wrap">
                          {[{ icon: ICONS.shield, text: de.payment.secureCheckout }, { icon: ICONS.refresh, text: de.payment.refundIfRejected }, { icon: ICONS.lock, text: de.payment.ssl }].map((b, i) => (
                    ...
                    <div className="space-y-4">
                      <div className="bg-emerald-50 rounded-2xl p-5 text-center border border-emerald-100">
                        ...
                        <p className="text-emerald-700 font-bold text-lg">{de.result.paymentSuccess}</p>
                        <p className="text-emerald-600 text-sm mt-1">{de.result.photoReady}</p>
                      </div>
                      <a href={`/api/download/${photoId}`} download={`photo-${documentType}.jpeg`} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2.5 transition-all text-sm shadow-lg shadow-blue-500/25">
                        <Icon d={ICONS.download} size={16} />{de.result.downloadDigital} {docCategory}
                      </a>
                      <a href={`/passport-photo-print-template-generator?imageUrl=${encodeURIComponent(`/api/download/${photoId}`)}&width=${spec?.width_mm || ""}&height=${spec?.height_mm || ""}&name=${encodeURIComponent(spec?.name || "")}`} className="w-full bg-lime-600 hover:bg-lime-700 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2.5 transition-all text-sm shadow-lg shadow-lime-500/20">
                        <Icon d={ICONS.photo} size={16} className="shrink-0" />Anpassen & Drucken (A4 / 10x15)
                      </a>
                      <a href={`/api/download-sheet/${photoId}`} download={`druckbogen-A4-${documentType}.jpeg`} className="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2.5 transition-all text-sm">
                        <Icon d={ICONS.photo} size={16} />{de.result.downloadPrintSheet} A4 Standard
                      </a>
                      <div className="grid grid-cols-2 gap-3">
                        <button onClick={handleEmailPhoto} className="bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold py-3 px-4 rounded-xl text-xs flex items-center justify-center gap-1.5"><Icon d={ICONS.mail} size={14} />{de.result.emailMe}</button>
                        <button className="bg-amber-50 hover:bg-amber-100 text-amber-700 font-bold py-3 px-4 rounded-xl text-xs flex items-center justify-center gap-1.5"><Icon d={ICONS.star} size={14} />{de.result.rateUs}</button>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="text-center">
                <Link href="/de/passbild-online" className="text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors">
                  {de.result.needAnother} <span className="underline">{de.result.uploadAgain}</span>
                </Link>
              </div>
            </div>
          </div>
        ...
        <>
          <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/96 backdrop-blur-xl border-t border-slate-200 shadow-[0_-8px_32px_rgba(0,0,0,0.12)] px-4 pt-3 pb-4 lg:hidden">
            <div className="max-w-lg mx-auto">
              {status !== "authenticated" && (
                <div className="mb-2.5"><input type="email" value={guestEmail || ""} onChange={(e) => setGuestEmail(e.target.value)} placeholder={de.payment.enterEmail} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all" /></div>
              )}
              <div className="flex items-center gap-3">
                ...
                <button onClick={onPaymentClick} disabled={loading}
                  ...
                  {loading ? (<div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />) : (<><Icon d={ICONS.download} size={16} />{isExpertPlan ? de.payment.expertReview : de.payment.downloadNow}</>)}
                </button>
              </div>
              <p className="text-center text-[10px] text-slate-400 mt-2">🔒 {de.payment.secureCheckout} · {de.payment.refundIfRejected} · {de.payment.noSubscription}</p>
            </div>
          </div>
