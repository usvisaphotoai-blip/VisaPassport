          ...
          <div className="flex items-center justify-between h-[60px] lg:h-[66px]">
            ...
            <div className="hidden lg:flex items-center gap-3 shrink-0">
              {status === "loading" ? (
                ...
                <>
                  ...
                  <button onClick={() => signOut({ callbackUrl: "/de" })} className="text-sm font-medium text-slate-400 hover:text-slate-700 transition-colors duration-150">
                    {de.nav.logout}
                  </button>
                  <Link href="/de/passbild-online" prefetch={true} className="bg-slate-900 text-white hover:bg-slate-800 active:bg-slate-950 rounded-xl px-5 py-2.5 text-sm font-bold transition-all duration-150 shadow-sm hover:shadow-md hover:shadow-black/10 hover:-translate-y-px">
                    {de.nav.newPhoto} →
                  </Link>
                </>
              ) : (
                <>
                  <Link href="/de/passbild-online" prefetch={true} className="bg-lime-600 text-white hover:bg-lime-700 active:bg-lime-800 rounded-xl px-5 py-2.5 text-sm font-bold transition-all duration-150 shadow-sm shadow-lime-600/25 hover:shadow-md hover:shadow-lime-600/30 hover:-translate-y-px whitespace-nowrap">
                    {de.nav.getPhoto} →
                  </Link>
                </>
              )}
            ...
            <div className="flex lg:hidden items-center gap-2">
              {status !== "loading" && !session && (
                <Link href="/de/passbild-online" className="bg-lime-600 text-white hover:bg-lime-700 rounded-lg px-3.5 py-2 text-xs font-bold transition-colors shadow-sm shadow-lime-600/20">
                  {de.nav.getPhoto}
                </Link>
              )}
              {status !== "loading" && session && (
                <Link href="/de/passbild-online" className="bg-slate-900 text-white hover:bg-slate-800 rounded-lg px-3.5 py-2 text-xs font-bold transition-colors shadow-sm">
                  {de.nav.newPhoto}
                </Link>
              )}
              <button
                ...
                <div className="mt-2 mx-1 flex flex-col gap-2">
                  {session ? (
                    <>
                      ...
                      <button onClick={() => { setMobileOpen(false); signOut({ callbackUrl: "/de" }); }} className="py-2.5 text-sm font-medium text-slate-400 hover:text-slate-700 transition-colors">
                        {de.nav.logout}
                      </button>
                    </>
                  ) : null}
                  <Link href="/de/passbild-online" onClick={() => setMobileOpen(false)} className="bg-lime-600 text-white text-center hover:bg-lime-700 active:bg-lime-800 rounded-xl px-5 py-3 text-sm font-bold transition-all shadow-sm shadow-lime-600/25">
                    {de.buttons.getApprovedPhoto}
                  </Link>
                </div>
              )}
            </div>
