...
export function validateBiometrics(
  ...
  imgWidth: number,
  imgHeight: number,
  spec: any // Raw response from the external API
): ValidationReport {
  ...
  metrics.file_format = {
    label: "File Format",
    value: imageFile.type.split("/")[1].toUpperCase(),
    status: ["image/jpeg", "image/jpg"].includes(imageFile.type) ? "success" : "fail"
  };
  ...
  if (!landmarks || landmarks.length === 0) {
    return {
      ...
      suggestions: ["Ensure your face is clearly visible and well-lit"],
      metrics: {
        face_detection: { label: "Face Detection", value: "Not detected", status: "fail" }
      }
    };
  }
  ...
  metrics.head_size = {
    ...
    status: (headHeightPct >= minHeadPct && headHeightPct <= maxHeadPct) ? "success" : "fail",
  ...
  const minEyePct = spec?.face_constraints?.eye_position?.from_bottom_pct?.min || 50;
  const maxEyePct = spec?.face_constraints?.eye_position?.from_bottom_pct?.max || 65;

  metrics.eye_level = {
    label: "Eye Level",
    value: `${eyeLevelPct.toFixed(1)}%`,
    status: (eyeLevelPct >= minEyePct && eyeLevelPct <= maxEyePct) ? "success" : "fail",
    target: `${minEyePct}-${maxEyePct}%`
  };
  ...
  const minChinToBottom = spec?.face_constraints?.chin_to_bottom_pct?.min || 5;
  metrics.chin_margin = {
    label: "Chin Margin",
  ...
  // Top Margin
  const topOfHeadY = foreheadY - (headHeightRaw * 0.34); // Estimate very top
  ...
  metrics.orientation = {
    ...
    status: (tiltAngle < 3 || tiltAllowed) ? "success" : (tiltAngle < 7 ? "warning" : "fail")
  };
  if (!tiltAllowed && tiltAngle >= 7) totalScore -= 15;

  // Eye Openness (using simple distance)
  const leftEyeOpen = Math.abs(landmarks[159].y - landmarks[145].y) > 0.01;
  const rightEyeOpen = Math.abs(landmarks[386].y - landmarks[374].y) > 0.01;
  ...
  metrics.eye_openness = {
    ...
    status: (!eyesOpenRequired || (leftEyeOpen && rightEyeOpen)) ? "success" : "fail"
  };
  if (eyesOpenRequired && metrics.eye_openness.status === "fail") totalScore -= 20;
  ...
  // Lighting & Background (Heuristics)
  const requiredBg = spec?.digital_requirements?.background_color || "plain";
  ...
  metrics.background = {
    label: "Background",
    value: requiredBg.charAt(0).toUpperCase() + requiredBg.slice(1),
    status: "success",
    target: requiredBg
  };

  // Summary generation
  if (totalScore >= 90) summary.push("Perfect biometric alignment");
  else if (totalScore >= 75) summary.push("Acceptable embassy quality");
  else summary.push("Significant compliance issues detected");

  if (metrics.head_size.status === "fail") suggestions.push("Adjust distance from camera to fix head size");
  if (metrics.eye_openness.status === "fail") suggestions.push("Ensure both eyes are clearly open");
  if (mouthOpen) suggestions.push("Keep a neutral expression with mouth closed");
